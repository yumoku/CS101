...
def numbers_in_lists(input_string: str):
    result = []
    i = 0

    while i < len(input_string):
        current = int(input_string[i])
        i += 1

        result.append(current)
        sublist = []
        while i < len(input_string) and int(input_string[i]) <= current:
            sublist.append(int(input_string[i]))
            i += 1
